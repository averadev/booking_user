...
local dbManager = {}
    ...
    dbManager.updateUser = function(idApp, email, password, name, apellido, condominio)
		...
		--verifica si el update fallo
		if result ~= 0 then
			query = "DROP TABLE IF EXISTS config;"
			db:exec( query )
			query = "CREATE TABLE IF NOT EXISTS config (id INTEGER PRIMARY KEY, idApp INTEGER, email TEXT, password TEXT, name TEXT, apellido TEXT, "..
		...
		openConnection( )
			...
			for i = 1, #items, 1 do
				...
				if result ~= 0 then
					...
					db:exec( query )
					query = "INSERT INTO condominios VALUES ('" .. items[i].condominioId .."', '" .. items[i].id .."', '" .. items[i].nameCondominious .."');"
					db:exec( query )
				end
			end
    ...
    dbManager.clearUser = function()
        openConnection( )
        query = "UPDATE config SET idApp = 0, email = '', password = '', name = '', apellido = '', condominioIdk = 0;"
        db:exec( query )
		query = "delete from condominios;"
        db:exec( query )
		query = "delete from residencial;"
